...
describe('Delta E JZ', () => {
  describe('Basic functionality', () => {
    ...
    it('should return 0 for identical colors', () => {
      const color = jzczhz(0.01, 0.02, 30);
  ...
  describe('Handling of undefined hue values', () => {
    it('should handle both hue values being undefined', () => {
      const color1 = jzczhz(0.01, 0.02, undefined as unknown as number);
      const color2 = jzczhz(0.02, 0.03, undefined as unknown as number);

      // When both hues are undefined, h1 and h2 should be set to 0
      // Manual calculation
      const ΔJ = 0.01 - 0.02;
      const ΔC = 0.02 - 0.03;
      const Δh = 0; // Both hues are set to 0
      const ΔH = 2 * Math.sqrt(0.02 * 0.03) * Math.sin(Δh * DEG_TO_RAD);
    ...
    it('should handle null hue values the same as undefined', () => {
      const color1 = jzczhz(0.01, 0.02, null as unknown as number);
      const color2 = jzczhz(0.02, 0.03, 60);
  ...
  describe('Edge cases', () => {
    it('should handle zero chroma values', () => {
      const color1 = jzczhz(0.01, 0, 30);
      const color2 = jzczhz(0.02, 0, 60);
    ...
    it('should handle extreme hue differences', () => {
      const color1 = jzczhz(0.01, 0.02, 0);
      const color2 = jzczhz(0.01, 0.02, 180);
    ...
    it('should handle alpha values if present (by ignoring them)', () => {
      const color1 = jzczhz(0.01, 0.02, 30, 1);
      const color2 = jzczhz(0.01, 0.02, 30, 0.5);
  ...
  describe('Implementation details', () => {
    it('should use the correct formula for the hue difference component', () => {
      const color1 = jzczhz(0.01, 0.02, 30);
      const color2 = jzczhz(0.01, 0.02, 60);
